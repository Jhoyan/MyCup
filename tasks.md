...
- [ ] **BE-019** 🟡 Reaproveitar jogador entre universos. Hoje [Player](backend/Models/Player.cs) tem `UniverseId` obrigatória e única (1 jogador = 1 universo); não há como adicionar um jogador existente em outro universo — só recriar do zero. Decidir a abordagem (decisão de modelagem do backend): (a) endpoint de **importar/copiar** jogador pro universo destino — cria registro novo, podendo linkar a mesma pessoa via `UserId` (baixo esforço, mantém o modelo); (b) **many-to-many** Jogador↔Universo (alto esforço — mexe em scoping, stats, soft-delete, listagens); (c) jogador **global** sem `UniverseId` (muda a premissa do produto). Quando houver endpoint, o front adiciona a UI (ex.: "Adicionar jogador de outro universo" na configuração/no universo).
