...
- **Authorization (BE-010 DONE):** RBAC via `UniverseAuthorizer` (see [[backend-progress]]). Reads are PUBLIC (`[AllowAnonymous]`); creating a universe makes the creator owner; members + content = admin; match results = moderator; universe delete + owner-role management = owner only. Roles ranked owner>admin>moderator; one role per user per universe (UserUniverse PK).
